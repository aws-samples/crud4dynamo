...
class ConfigTest {
    ...
    void mergeMapperConfig() {
        ...
        AssertionsForClassTypes.assertThat(mergedConfig.mapperConfig()).isEqualTo(DynamoDBMapperConfig.DEFAULT);
    ...
    void mergeFactoryConstructors() {
        ...
        final Config base = Config.builder().crudFactoryConstructorConfig(config1).crudFactoryConstructorConfig(config3).build();
    ...
    void mergeTransactionFactoryConstructors() {
        ...
        final Config base =
                Config.builder().transactionFactoryConstructorConfig(config1).transactionFactoryConstructorConfig(config3).build();
        final Config overrides = Config.builder().transactionFactoryConstructorConfig(config2).build();

        final Config mergedConfig = Config.merge(base, overrides);

        assertThat(mergedConfig.transactionFactoryConstructorConfigs()).containsExactly(config1, config2);
    }
