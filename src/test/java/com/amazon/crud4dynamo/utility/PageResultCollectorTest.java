    ...
    private static class PageRequestMocker<T> {
        ...
        public PageRequestMocker<T> mockOnePage(final T... obj) {
            ...
            final PageRequest<T> request =
                    PageRequest.<T>builder().limit(DUMMY_MAX_PAGE_SIZE).exclusiveStartItem(lastEvaluatedItem).build();
            ...
            final PageResult<T> result = PageResult.<T>builder().items(items).lastEvaluatedItem(obj[obj.length - 1]).build();
        ...
        public PageRequestMocker<T> finish() {
            final PageRequest<T> request =
                    PageRequest.<T>builder().limit(DUMMY_MAX_PAGE_SIZE).exclusiveStartItem(lastEvaluatedItem).build();
            when(requester.issue(request)).thenReturn(PageResult.<T>builder().lastEvaluatedItem(null).items(new ArrayList<>()).build());
            return this;
        }
    }
}
