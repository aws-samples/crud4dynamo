...
class DynamoDbMapperAnnotationCompatibleTest {

  @Nested
  class BasicCrudTest extends SingleTableDynamoDbTestBase<Book> {
    ...
    void annotation_DynamoDBTyped() {
      ...
      final Book book = books.get(0);
      final Optional<Item> item = getItem(
          new GetItemSpec().withPrimaryKey(Attributes.HASH_KEY, dummyAuthor, Attributes.RANGE_KEY,
              book.getId()));
    ...
    void annotation_DynamoDBTypeConverted() {
      final String dummyAuthor = "dummy author";
      final Book bookItem = Book.builder().author(dummyAuthor).customDate(CustomDate.builder()
          .year(1984)
          .month(4)
          .day(1).build())
          .build();
      ...
      assertThat(item).isPresent();
      assertThat(item.get().asMap())
          .containsEntry(Attributes.CUSTOM_DATE, "1984-4-1");
    }
  }
}
