...
public class QueryAndScanMapperAnnotationTest extends SingleTableDynamoDbTestBase<Book> {
  ...
  public interface BookDao extends CompositeKeyCrud<String, Integer, Book> {
    ...
    @Query(keyCondition = Attributes.HASH_KEY + " = :hashKey",
  ...
  void query_annotation_DynamoDBTyped() {
    ...
    final Book item1 = Book.builder()
        .author(dummyAuthor)
        .integerStoredAsString(dummyInteger1)
        .build();
    bookDao.save(item1);
    final Book item2 = Book.builder()
        .author(dummyAuthor)
        .integerStoredAsString(dummyInteger2)
        .build();
    bookDao.save(item2);

    final List<Book> books = Lists
        .newArrayList(bookDao.queryFilterByIntegerStoredAsString(dummyAuthor, dummyInteger1));
  ...
  void scan_annotation_DynamoDBTyped() {
    ...
    final List<Book> books = Lists
        .newArrayList(bookDao.filterByIntegerStoredAsString(dummyInteger1));
  ...
  void query_annotation_DynamoDBTypeConverted() {
    ...
    final CustomDate customDate = CustomDate.builder()
        ...
        .month(4)
        .day(1).build();
    final Book bookItem = Book.builder().author(dummyAuthor).customDate(customDate)
        .build();
    bookDao.save(bookItem);

    final List<Book> books = Lists
        .newArrayList(bookDao.queryFilterByCustomDate(dummyAuthor, customDate));
    assertThat(books).hasSize(1);
  ...
  void query_annotation_DynamoDBDocument() {
    final String dummyAuthor = "dummy author";
    final Picture cover = Picture.builder()
        .height(100)
        .width(100)
        .sourcePath("a/path")
        .build();
    final Book bookItem = Book.builder()
        .author(dummyAuthor)
        .cover(cover)
        .build();
    bookDao.save(bookItem);
