...
public class PutMethodsMapperAnnotationTest extends SingleTableDynamoDbTestBase<Book> {
  ...
  void annotation_DynamoDBAutoGeneratedKey_throwException() {
    assertThatThrownBy(() -> bookDao.put(Book.builder()
        .author("dummy")
        .build())).hasMessageContaining("One of the required keys was not given a value");
  }
  ...
  void annotation_DynamoDBTypeConverted() {
    final String dummyAuthor = "dummy author";
    final Book book = Book.builder().author(dummyAuthor)
        .id("dummy id")
        .customDate(CustomDate.builder()
        .year(1984)
        .month(4)
        .day(1).build())
        .build();
