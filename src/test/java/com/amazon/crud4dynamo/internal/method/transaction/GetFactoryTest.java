...
class GetFactoryTest extends SingleTableDynamoDbTestBase<GetFactoryTest.Table> {
  ...
  void create() throws Exception {
    ...
    final GetFactory factory =
        new GetFactory(signature.getAnnotation(Get.class).get(), getDynamoDbMapperTableModel());

    final com.amazonaws.services.dynamodbv2.model.Get get =
        factory.create(
            Argument.newList(
                signature.parameters(), Arrays.asList(hashKey, Table.STRING_ATTRIBUTE)));
