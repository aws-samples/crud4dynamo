...
class PutRequestFactoryTest extends SingleTableDynamoDbTestBase<PutRequestFactoryTest.Model> {
  ...
  void method_withNonNonReturnValue_and_voidReturnType_throwException() throws Exception {
    assertThatThrownBy(
            () -> newFactory(Dao.class.getMethod("method_withNotNonReturnValue", Model.class)))
  ...
  void method_withCustomPut() throws Exception {
    final PutRequestFactory factory =
        newFactory(
            Dao.class.getMethod("method_withCustomPut", Model.class, String.class, String.class));
    final Model model = Model.builder().hashKey("hashKey").rangeKey(1).build();
        ...
        .isEqualTo(
            new PutItemRequest()
                ...
                .withExpressionAttributeValues(
                    ImmutableMap.of(":value", new AttributeValue(dummyAttributeValue)))
