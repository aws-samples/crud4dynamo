...
class DeleteFactoryTest extends SingleTableDynamoDbTestBase<DeleteFactoryTest.Table> {
  ...
  void delete() throws Exception {
    final Method deleteMethod =
        Dao.class.getMethod("delete", String.class, String.class, String.class);
    final Signature signature = Signature.resolve(deleteMethod, Dao.class);
    final DeleteFactory factory =
        new DeleteFactory(
            signature.getAnnotation(Delete.class).get(), getDynamoDbMapperTableModel());
    ...
    final com.amazonaws.services.dynamodbv2.model.Delete delete =
        factory.create(
            Argument.newList(
                ...
                Arrays.asList(hashKeyValue, Table.STRING_ATTRIBUTE, prefix)));
    ...
    assertThat(delete.getExpressionAttributeNames())
        .containsEntry("#attribute", Table.STRING_ATTRIBUTE);
    assertThat(delete.getExpressionAttributeValues())
