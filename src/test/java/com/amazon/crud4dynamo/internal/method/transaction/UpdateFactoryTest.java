...
class UpdateFactoryTest extends SingleTableDynamoDbTestBase<UpdateFactoryTest.Table> {
  ...
  void update() throws Exception {
    final Method method =
        Dao.class.getMethod("update", String.class, String.class, String.class, String.class);
    final Signature signature = Signature.resolve(method, Dao.class);
    final UpdateFactory factory =
        new UpdateFactory(
            signature.getAnnotation(Update.class).get(), getDynamoDbMapperTableModel());
    ...
    final com.amazonaws.services.dynamodbv2.model.Update update =
        factory.create(
            Argument.newList(
                ...
                Arrays.asList(hashKeyValue, Table.STRING_ATTRIBUTE, newValue, oldValue)));

    assertThat(update.getTableName()).isEqualTo(Table.NAME);
    assertThat(update.getKey())
        .containsEntry(Table.HASH_KEY, new AttributeValue(hashKeyValue))
        .hasSize(1);
    ...
    assertThat(update.getExpressionAttributeNames())
