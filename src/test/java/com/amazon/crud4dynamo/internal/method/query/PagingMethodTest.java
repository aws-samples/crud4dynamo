...
class PagingMethodTest extends SingleTableDynamoDbTestBase<PagingMethodTest.Model> {
    ...
    private interface Dao extends CompositeKeyCrud<String, Integer, Model> {
        ...
        PageResult<Model> query(@Param(":hashKey") final String hashKey, final PageRequest<Model> request);
    }
    ...
    void invoke() throws Throwable {
        ...
        final List<Model> models = PageResultCollector.newCollector(getInitialRequest(), getRequester(queryMethod)).get();
    ...
    private PagingMethod getQueryMethod() throws NoSuchMethodException {
        ...
        return new PagingMethod(Signature.resolve(method, Dao.class), getModelClass(), getDynamoDbMapper(), null);
    }

    private List<Model> prepareTestModels() {
        return storeItems(IntStream.range(0, 10).mapToObj(i -> Model.builder().hashKey(GROUP_KEY).rangeKey(i).build()));
    }
}
