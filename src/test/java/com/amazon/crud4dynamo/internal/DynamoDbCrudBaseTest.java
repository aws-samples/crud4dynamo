...
public class DynamoDbCrudBaseTest extends SingleTableDynamoDbTestBase<Model> {
    ...
    public void setUp() throws Exception {
        super.setUp();
        dao = new DynamoDbCrudBase<Model>(getDynamoDbMapper(), DynamoDBMapperConfig.DEFAULT, Model.class) {};
    }
    ...
    private PageResultCollector<Model, Model> newPageResultCollector() {
        return PageResultCollector.newCollector(
                PageRequest.<Model>builder().exclusiveStartItem(null).limit(1).build(), req -> dao.findAll(req));
    }
}
