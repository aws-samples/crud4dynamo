...
class ExpressionAttributesFactoryTest {
  ...
  void constructWithMultipleParsers() throws Exception {
    final List<Argument> arguments =
        TestInterface.newArgumentList(
            TestInterface.getMethodA(), "name1", "name2", "name3", "value1", "value2", "value3");

    final ExpressionAttributesFactory factory =
        new ExpressionAttributesFactory(new TestParser1(), new TestParser2());
  ...
  private class TestParser1 implements ExpressionParser {
    ...
    public AttributeValueMapper getAttributeValueMapper() {
      return new AttributeValueMapper()
          .put(EXPRESSION_ATTRIBUTE_VALUE_1, object -> new AttributeValue(object.toString()));
    }
  ...
  private class TestParser2 implements ExpressionParser {
    ...
    public AttributeValueMapper getAttributeValueMapper() {
      return new AttributeValueMapper()
          .put(EXPRESSION_ATTRIBUTE_VALUE_2, object -> new AttributeValue(object.toString()));
    }
