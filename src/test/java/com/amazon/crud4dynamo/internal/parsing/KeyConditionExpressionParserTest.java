...
class KeyConditionExpressionParserTest {
  ...
  class CompareExpression extends SingleTableDynamoDbTestBase<ModelWithIntegerRangeKey> {
    ...
    private final ImmutableMap<String, Object> arguments =
        ImmutableMap.of(":hashKey", "hashkey", ":lower", 10);
    private final AttributeValueMapper expectedValueMapper = new AttributeValueMapper();

    {
      expectedValueMapper.put(
          ":hashKey", obj -> getDynamoDbMapperTableModel().hashKey().convert(obj));
      expectedValueMapper.put(
          ":lower", obj -> getDynamoDbMapperTableModel().rangeKey().convert(obj));
    }
    ...
    void withAttributeName() {
      ...
      final KeyConditionExpressionParser mapper =
          new KeyConditionExpressionParser(keyCondition, getDynamoDbMapperTableModel());
  ...
  class BetweenExpression extends SingleTableDynamoDbTestBase<ModelWithIntegerRangeKey> {
    ...
    private final ImmutableMap<String, Object> arguments =
        ImmutableMap.of(":hashKey", "hashkey", ":lower", 10, ":upper", 20);
    private final AttributeValueMapper expectedValueMapper = new AttributeValueMapper();

    {
      ...
      expectedValueMapper.put(
          ":lower", obj -> getDynamoDbMapperTableModel().rangeKey().convert(obj));
      expectedValueMapper.put(
          ":upper", obj -> getDynamoDbMapperTableModel().rangeKey().convert(obj));
    }
  ...
  class BeginsWithExpression extends SingleTableDynamoDbTestBase<ModelWithStringRangeKey> {
    ...
    private final ImmutableMap<String, Object> arguments =
        ImmutableMap.of(":hashKey", "hashkey", ":prefix", "Prefix");
    private final AttributeValueMapper expectedValueMapper = new AttributeValueMapper();

    {
      expectedValueMapper.put(
          ":hashKey", obj -> getDynamoDbMapperTableModel().hashKey().convert(obj));
      expectedValueMapper.put(
          ":prefix", obj -> getDynamoDbMapperTableModel().rangeKey().convert(obj));
    }
