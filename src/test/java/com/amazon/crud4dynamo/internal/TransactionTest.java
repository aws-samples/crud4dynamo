...
public class TransactionTest extends DynamoDbTestBase {
    ...
    private interface Transaction {
        ...
        @Put(tableClass = Orders.class, item = ":newOrder", conditionExpression = "attribute_not_exists(OrderId)")
        void write(
        ...
        @Get(tableClass = Customer.class, keyExpression = "CustomerId = :customerId", projectionExpression = "CustomerId")
        @Get(tableClass = ProductCatalog.class, keyExpression = "ProductId = :productId", projectionExpression = "ProductId, ProductStatus")
        @Get(tableClass = Orders.class, keyExpression = "OrderId = :orderId", projectionExpression = "OrderId,OrderStatus,OrderTotal")
        @Get(tableClass = Customer.class, keyExpression = "CustomerId = :nonExistingCustomerId", projectionExpression = "CustomerId")
        List<Object> get(
                @Param(":customerId") final String customerId,
                @Param(":productId") final String productId,
    ...
    class WriteTest {
        ...
        private final Orders newOrder = Orders.builder().id("new order random id").status("CONFIRMED").total(100).build();
        ...
        void write() {
            ...
            getDbMapper().save(ProductCatalog.builder().id(productId).status(expectedOldProductStatus).build());

            transaction.write(customerId, productId, newProductStatus, expectedOldProductStatus, newOrder);

            assertThat(getDbMapper().load(ProductCatalog.builder().id(productId).build()).getStatus()).isEqualTo(newProductStatus);
            assertThat(getDbMapper().load(Orders.builder().id(newOrder.getId()).build())).isEqualTo(newOrder);
        }
    }
    ...
    class GetTest {
        ...
        private final ProductCatalog productCatalog = ProductCatalog.builder().id("product id").status("SOLD").build();
        private final Orders order = Orders.builder().id("new order random id").status("CONFIRMED").total(100).build();
        ...
        void get() {
            ...
            final List<Object> retrievedItems =
                    transaction.get(customer.getId(), productCatalog.getId(), order.getId(), "nonExistingCustomerId");
