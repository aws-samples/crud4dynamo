...
class MapperConfigAwareMethodFactoryTest {
  ...
  void methodWithDefaultMapperConfigAnnotation() throws Exception {
    ...
    assertThat(actualContext.mapperConfig())
        .matches(newConfigPredicate(ConsistentReads.EVENTUAL, SaveBehavior.UPDATE));
  }
  ...
  void methodWithCustomMapperConfigAnnotation() throws Exception {
    ...
    assertThat(actualContext).isEqualToIgnoringGivenFields(context, "mapperConfig");
    assertThat(actualContext.mapperConfig())
        .matches(newConfigPredicate(ConsistentReads.CONSISTENT, SaveBehavior.APPEND_SET));
  }

  private Predicate<DynamoDBMapperConfig> newConfigPredicate(
      ConsistentReads consistent, SaveBehavior appendSet) {
    return config ->
        config.getConsistentReads().equals(consistent)
            && config.getSaveBehavior().equals(appendSet);
  }
