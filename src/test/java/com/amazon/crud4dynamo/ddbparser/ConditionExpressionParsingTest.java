...
class ConditionExpressionParsingTest extends ParsingTestBase {
    ...
    Class<? extends Lexer> getLexerClass() {
        return ConditionExpressionLexer.class;
    }

    @Override
    Class<? extends Parser> getParserClass() {
        return ConditionExpressionParser.class;
    }
    ...
    void InExpressionHappyCase() {
        parse("#attr IN (:val1, :val2, :val3)", "(start (conditionExpression (path #attr) IN ( :val1 , :val2 , :val3 )) <EOF>)");
    }
    ...
    void attributeTypeFunctionHappyCase() {
        parse("attribute_type(A, :type)", "(start (conditionExpression (functionExpression attribute_type ( (path A) , :type ))) <EOF>)");
    }
    ...
    void beginsWithFunctionHappyCase() {
        parse("begins_with(#abc, :value)", "(start (conditionExpression (functionExpression begins_with ( (path #abc) , :value ))) <EOF>)");
    }
    ...
    void containsExpressionHappyCase() {
        parse("contains(A, :val)", "(start (conditionExpression (functionExpression contains ( (path A) , :val ))) <EOF>)");
    }
