...
public class GetFactory {
  ...
  public GetFactory(
      final com.amazon.crud4dynamo.annotation.transaction.Get getAnnotation,
      final DynamoDBMapperTableModel<?> tableModel) {
    this.getAnnotation = getAnnotation;
    this.tableModel = tableModel;
    keyAttributeConstructor =
        new KeyAttributeConstructor(getAnnotation.keyExpression(), tableModel);
    expressionAttributesFactory =
        new ExpressionAttributesFactory(
            new ProjectionExpressionParser(getAnnotation.projectionExpression()));
  }
