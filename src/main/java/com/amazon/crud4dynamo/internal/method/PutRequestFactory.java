...
class PutRequestFactory {
  ...
  PutRequestFactory(
      ...
      @NonNull final DynamoDBMapper dbMapper) {
  ...
  private static Signature checkSignatureOrThrow(
      final Signature signature, final Class<?> modelClass) {
    ...
    if (returnValue != ReturnValue.NONE
        && !signature.invokable().getReturnType().equals(TypeToken.of(modelClass))) {
      throw new ReturnTypeInvalidException(signature, returnValue, modelClass);
    }
    return signature;
  }

  private static boolean noParameterAnnotatedAsPutItem(
      final Signature signature, final Put putAnnotation) {
    return signature.invokable().getParameters().stream()
  ...
  private Object getPutItem(final List<Argument> args) {
    return args.stream()
        .filter(filterArgumentByPutItem())
        .findFirst()
  ...
  static class NoPutAnnotationException extends CrudForDynamoException {
    private NoPutAnnotationException(final Signature signature) {
      super(
          String.format(
              "Method '%s' is not annotated with @%s", signature, Put.class.getSimpleName()));
    }
  }
  ...
  static class NoPutItemAnnotationException extends CrudForDynamoException {
    private NoPutItemAnnotationException(final Signature signature, final Put put) {
      super(
          String.format(
              "Method '%s' does not have a parameter annotated with @Param(%s)",
              signature, put.item()));
    }
  }
