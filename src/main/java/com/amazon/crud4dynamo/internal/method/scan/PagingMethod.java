...
public class PagingMethod implements AbstractMethod {
  ...
  public PagingMethod(
    ...
    expressionFactory =
        new PagingExpressionFactory(
            new NonPagingExpressionFactory(signature, modelType, dynamoDbMapper),
            modelType,
            dynamoDbMapper);
  }
  ...
  public Object invoke(final Object... args) throws Throwable {
    final ScanResultPage<?> result =
        dynamoDbMapper.scanPage(modelType, expressionFactory.create(args), mapperConfig);
    return PageResult.builder()
        .items(result.getResults())
        .lastEvaluatedItem(getLastEvaluatedKey(result))
        .build();
  }
