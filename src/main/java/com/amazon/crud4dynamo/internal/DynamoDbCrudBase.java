...
public abstract class DynamoDbCrudBase<M> implements DynamoDbCrud<M> {
  ...
  public FailedBatch<M> saveAll(final Iterable<M> models) {
    return new FailedBatchConverter<M>(
        dynamoDbMapper.batchWrite(models, Collections.emptyList(), mapperConfig)) {
      @Override
  ...
  public FailedBatch<M> deleteAll(final Iterable<M> models) {
    return new FailedBatchConverter<M>(
        dynamoDbMapper.batchWrite(Collections.emptyList(), models, mapperConfig)) {
      @Override
      protected List<M> transform(final Stream<WriteRequest> writeRequestStream) {
        final List<Map<String, AttributeValue>> keyAttributeList =
            writeRequestStream
                .map(WriteRequest::getDeleteRequest)
                .map(DeleteRequest::getKey)
                .collect(Collectors.toList());
        return filterToFindFailedModels(keyAttributeList);
      }

      private List<M> filterToFindFailedModels(
          final List<Map<String, AttributeValue>> keyAttributeList) {
        return Lists.newArrayList(models).stream()
            .filter(
                model ->
                    keyAttributeList.stream()
                        .anyMatch(
                            keyAttribute -> keyAttribute.equals(tableModel.convertKey(model))))
            .collect(Collectors.toList());
      }
    }.convert();
  }
    ...
    return dynamoDbMapper
        .scan(
            ...
            DynamoDbMapperConfigHelper.override(
                mapperConfig, PaginationLoadingStrategy.ITERATION_ONLY.config()))
        .iterator();
  }
        ...
        new DynamoDBScanExpression()
            ...
            .withExclusiveStartKey(
                Optional.ofNullable(pageRequest.getExclusiveStartItem())
                    ...
                    .orElse(null));
    final ScanResultPage<M> scanResultPage =
        dynamoDbMapper.scanPage(modelClass, expression, mapperConfig);
    return PageResult.<M>builder()
        .items(scanResultPage.getResults())
        .lastEvaluatedItem(
            Optional.ofNullable(scanResultPage.getLastEvaluatedKey())
                .map(tableModel::unconvert)
                .orElse(null))
        .build();
  }
  ...
  private abstract class FailedBatchConverter<M> {
    ...
    private List<FailedBatch.SubBatch<M>> buildSubBatches(
        final Stream<DynamoDBMapper.FailedBatch> failedBatchStream) {
    ...
    private FailedBatch.SubBatch<M> buildSubBatch(final DynamoDBMapper.FailedBatch failedBatch) {
      final Stream<WriteRequest> writeRequestStream =
          failedBatch.getUnprocessedItems().values().stream().flatMap(List::stream);
