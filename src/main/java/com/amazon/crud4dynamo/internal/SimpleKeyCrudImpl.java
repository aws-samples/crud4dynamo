...
public class SimpleKeyCrudImpl<H, M> extends DynamoDbCrudBase<M> implements SimpleKeyCrud<H, M> {
    ...
    public SimpleKeyCrudImpl(final DynamoDBMapper dynamoDbMapper, final DynamoDBMapperConfig mapperConfig, final Class<M> modelClass) {
    ...
    public FailedBatch<H> deleteAllBy(final Iterable<H> hashKeys) {
        ...
        return FailedBatch.<H>builder().subBatches(convertToKeySubBatches(tableModel, modelFailedBatch)).build();
    }
    ...
    private List<H> convertToHashKeys(final FailedBatch.SubBatch<M> modelSubBatch, final DynamoDBMapperTableModel<M> tableModel) {
        return modelSubBatch.getFailedItems().stream().map(model -> tableModel.<H>hashKey().get(model)).collect(Collectors.toList());
    }
    ...
    public Iterator<M> findAllBy(final Iterable<H> hashKeys) {
        final List<M> models = convertHashKeysToModels(hashKeys);
        return (Iterator<M>) dynamoDbMapper.batchLoad(models, mapperConfig).values().stream().flatMap(Collection::stream).iterator();
    }
