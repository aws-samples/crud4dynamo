...
public class KeyExpressionMapper {
    ...
    public KeyExpressionMapper(final String keyExpression, final DynamoDBMapperTableModel tableModel) {
    ...
    public Context getHashKeyContext() {
        ...
        return Context.builder().keyStringText(hashKeyText).nameMapper(nameMapper).valueMapper(valueMapper).build();
    }

    private String doVisit(
            final EqualityExpressionContext equalExpr, final AttributeNameMapper nameMapper, final AttributeValueMapper valueMapper) {
        ...
        return Optional.ofNullable(equalExpr.ATTRIBUTE_NAME())
                ...
                .orElseGet(() -> Optional.ofNullable(equalExpr.EXPRESSION_ATTRIBUTE_NAME()).map(ParseTree::getText).get());
    }

    private KeyExpressionBaseVisitor<Void> newValueMapperVisitor(final AttributeValueMapper valueMapper) {
            ...
            public Void visitEqualityExpression(final EqualityExpressionContext ctx) {
                Optional.ofNullable(ctx.ATTRIBUTE_NAME())
                        ...
                        .ifPresent(
                                attributeName ->
                                        valueMapper.put(
                                                ctx.EXPRESSION_ATTRIBUTE_VALUE().getText(), tableModel.field(attributeName)::convert));
                return null;
            }
        };
    }

    private KeyExpressionBaseVisitor<Void> newNameMapperVisitor(final AttributeNameMapper nameMapper) {
        return new KeyExpressionBaseVisitor<Void>() {
            @Override
            public Void visitEqualityExpression(final EqualityExpressionContext ctx) {
                Optional.ofNullable(ctx.EXPRESSION_ATTRIBUTE_NAME())
                        .map(ParseTree::getText)
                        .ifPresent(
                                placeholder -> nameMapper.put(placeholder, newLazyConverter(ctx.EXPRESSION_ATTRIBUTE_VALUE().getText())));
    ...
    public Optional<Context> getRangeKeyContext() {
        ...
        return keyText.map(text -> Context.builder().keyStringText(text).nameMapper(nameMapper).valueMapper(valueMapper).build());
    }
    ...
    private Optional<StartContext> getContextRoot() {
        return Optional.ofNullable(keyExpression)
                ...
                .map(expr -> new ParserFactory<>(KeyExpressionLexer.class, KeyExpressionParser.class).create(expr))
