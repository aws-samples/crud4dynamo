 ...
 *
 * <p>its key is the expression attribute name, e.g, "#HashKey" its value is a list of function, which when applied with attribute name,
 * e.g, "HashKey", return an attribute value converter.
 */
@EqualsAndHashCode
public class AttributeNameMapper {
    ...
    public static AttributeNameMapper merge(final AttributeNameMapper m1, final AttributeNameMapper m2) {
        final Map<String, List<Function<String, NameAwareConverter>>> map =
                MapHelper.merge(m1.innerMap, m2.innerMap).entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, flatList()));
        return new AttributeNameMapper(map);
    }

    public AttributeNameMapper put(final String attrName, final Function<String, NameAwareConverter> converterFunction) {
        final List<Function<String, NameAwareConverter>> list = innerMap.getOrDefault(attrName, new ArrayList<>());
    ...
    private static Function<Map.Entry<String, List<List<Function<String, NameAwareConverter>>>>, List<Function<String, NameAwareConverter>>>
            flatList() {
        return entry -> entry.getValue().stream().flatMap(Collection::stream).collect(Collectors.toList());
    }
    ...
    /** Convert a map whose key is an expression attribute name and value is the corresponding name to AttributeValueMapper. */
